...
fpds_get_data <- function(piid=NULL,
  ...
  if(!is.null(piid)){
    ...
    if(verbose==T)message(paste("Processing",calc,"PIID values."))
    for(i in 1:calc){
      if(length(calc)>1)
      ...
      if (is.null(x)) {
        message(paste("Skipping PIID:", piid_val, "due to API failure"))
        next
      ...
      if(length(startn)>0){
        ...
        for(lp in 1:loops){
          ...
          if (is.null(xx)) {
            ...
            next
            failLoops <- failLoops+1
            failTable[failLoops,] <- c(piid_val,)
          }
    ...
    if(!is.null(dl$IDV)){
      ...
      clocs <- grep("countryCode",names(dl$IDV))
      ...
      names(dl$IDV)[clocs[1]] <- "countryCode"
      names(dl$IDV)[clocs[2]] <- "idvCountryCode"
    }
...
fpds_get_data_new <- function(piid=NULL,
                              return="xml",
                              include_related=FALSE,
                              verbose=TRUE){
