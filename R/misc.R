...
#   ## JOIN PHASE A
#   if(verbose==T)message("Joining by PIID and modNumber.")
#   ia <- inner_join(dc,fa,by=c("PIID","modNumber","vendor_clean","description_standard"))
#   ii <- inner_join(dc,fi,by=c("PIID","modNumber","vendor_clean","description_standard"))
#                    
#   ia_piids <- ia$PIID
#   ii_piids <- ii$PIID
#   
...
#  nrow AWARDS: ",stats[1],"
#  nrow IDV: ",stats[2],"
#  unique PIIDS Awards: ",stats[3],"
#  unique PIIDS Awards: ",stats[4]," 
#  Probable Duplicates: ",(stats[1]+stats[2])-(stats[3]+stats[4]),"\n\n")
#   
#   ### STEP THREE
#   ## JOIN PHASE B
#   # This shows what's not getting matched by PIID, modNumber, and vendorName. 
#   ia_piids_notmerged <- unique(uniquePIIDS_awards[!(uniquePIIDS_awards %in% ia_piids)])
#   ii_piids_notmerged <- unique(uniquePIIDS_idv[!(uniquePIIDS_idv %in% ii_piids)])
#   
#   unmatched_AWARDS <- fa[fa$PIID %in% ia_piids_notmerged,]
#   unmatched_IDV <- fi[fi$PIID %in% ii_piids_notmerged,]
#   
#   ia_limit <- inner_join(dc,unmatched_AWARDS,by=c("PIID","modNumber","vendor_clean"))
#   ii_limit <- inner_join(dc,unmatched_IDV,by=c("PIID","modNumber","vendor_clean"))
#   
#   stats_limit <- c(nrow(ia_limit),
#                    nrow(ii_limit),
#                    length(unique(ia_limit$PIID)),
#                    length(unique(ii_limit$PIID)))
#   
#   cat("JOIN PHASE B:
#  nrow AWARDS: ",stats_limit[1],"
#  nrow IDV: ",stats_limit[2],"
#  unique PIIDS Awards: ",stats_limit[3],"
#  unique PIIDS Awards: ",stats_limit[4]," 
#  Probable Duplicates: ",(stats_limit[1]+stats_limit[2])-(stats_limit[3]+stats_limit[4]),"
#  Total Accounted for PIIDS: ",stats[3]+stats[4]+stats_limit[3]+stats_limit[4],"
#  Unaccounted For PIIDS: ",(length(uniquePIIDS_total)-(stats[3]+stats[4]+stats_limit[3]+stats_limit[4])),"\n\n")
#   
#   ### STEP FOUR
#   ## JOIN PHASE C
#   ia_still_no <- ia_piids_notmerged[!(ia_piids_notmerged %in% ia_limit$PIID)]
#   ii_still_no <- ii_piids_notmerged[!(ii_piids_notmerged %in% ii_limit$PIID)]
#   
#   unmatched_AWARDS <- unmatched_AWARDS[unmatched_AWARDS$PIID %in% ia_still_no,]
#   unmatched_IDV <- unmatched_IDV[unmatched_IDV$PIID %in% ii_still_no,]
#   
#   ia_limit_B <- inner_join(dc,unmatched_AWARDS,by=c("PIID","modNumber","description_standard"))
#   ii_limit_B <- inner_join(dc,unmatched_IDV,by=c("PIID","modNumber","description_standard"))
#   
#   stats_limit_B <- c(nrow(ia_limit_B),
#                    nrow(ii_limit_B),
#                    length(unique(ia_limit_B$PIID)),
#                    length(unique(ii_limit_B$PIID)))
#   
#   cat("JOIN PHASE C:
#  nrow AWARDS: ",stats_limit_B[1],"
#  nrow IDV: ",stats_limit_B[2],"
#  unique PIIDS Awards: ",stats_limit_B[3],"
#  unique PIIDS Awards: ",stats_limit_B[4]," 
#  Probable Duplicates: ",(stats_limit_B[1]+stats_limit_B[2])-(stats_limit_B[3]+stats_limit_B[4]),"
#  Total Accounted for PIIDS: ",stats[3]+stats[4]+stats_limit[3]+stats_limit[4]+stats_limit_B[3]+stats_limit_B[4],"
#  Unaccounted For PIIDS: ",(length(uniquePIIDS_total)-(stats[3]+stats[4]+stats_limit[3]+stats_limit[4]+stats_limit_B[3]+stats_limit_B[4])),"\n\n")
#   
#   ### STEP FIVE
#   dupsa <- ia %>% count(PIID,modNumber,vendor_clean) %>% filter(n>1)
#   dupsi <- ii %>% count(PIID,modNumber,vendor_clean) %>% filter(n>1)
#   dupsal <- ia_limit %>% count(PIID,modNumber,description_standard) %>% filter(n>1)
#   dupsil <- ii_limit %>% count(PIID,modNumber,description_standard) %>% filter(n>1)
#   dupsalb <- ia_limit_B %>% count(PIID,modNumber) %>% filter(n>1)
#   dupsilb <- ii_limit_B %>% count(PIID,modNumber) %>% filter(n>1)
#   
#   ## Delete duplicates and try to do a final join to eliminate dups. 
#   ia_limit_B_clean <- ia_limit_B[!(ia_limit_B$PIID %in% dupsalb$PIID),]
#   ii_limit_B_clean <- ii_limit_B[!(ii_limit_B$PIID %in% dupsalb$PIID),]
#   ia_limit_C <- ia_limit_B[ia_limit_B$PIID %in% dupsalb$PIID,]
#   ia_limit_C <- ii_limit_B[ii_limit_B$PIID %in% dupsilb$PIID,]
#   
#   stop()
#   
#   ia_limit_C <- inner_join(dc,ia_limit_C,by=c("PIID","modNumber","description_standard","vendor_clean"))
#   ii_limit_C <- inner_join(dc,ii_limit_C,by=c("PIID","modNumber","description_standard","vendor_clean"))
#   
#   duplicateLists <- list(P1A = dupsa,
#                          P1I = dupsi,
#                          P2A = dupsal,
#                          P2I = dupsil,
#                          P3A = dupsalb,
#                          P3I = dupsilb)
#   
#   if(returnDuplicates==TRUE)
#     return(duplicateLists)
#   
#   ### STEP SIX 
#   ia_still_no <- ia_still_no[!(ia_still_no %in% ia_limit_B$PIID)]
#   ii_still_no <- ii_still_no[!(ii_still_no %in% ii_limit_B$PIID)]
#   
#   unmatched_AWARDS <- unmatched_AWARDS[unmatched_AWARDS$PIID %in% ia_still_no,]
#   unmatched_IDV <- unmatched_IDV[unmatched_IDV$PIID %in% ii_still_no,]
#   
...
#   print(nrow(ia_limit))
#   print(nrow(ii_limit))
#   print(length(unique(ia_limit$PIID)))
#   print(length(unique(ii_limit$PIID)))
#   print(length(ia_still_no))
#   print(length(ii_still_no))
#   print(nrow(ia_limit_B))
#   print(nrow(ii_limit_B))
#   
#   cat("Summary:\n
#  DOGE Starting Unique PIIDs: ",length(unique(dc$PIID)),"
#  FPDS:AWARDS Starting Nrow: ",nrow(fa),"
#  FPDS:IDV Starting Nrow: ",nrow(fa),"
#  FPDS:AWARDS Starting Unique PIIDs: ",length(uniquePIIDS_awards),"
#  FPDS:IDV Starting Unique PIIDs: ",length(uniquePIIDS_idv),"
#  Total Unique Starting PIIDs: ",length(uniquePIIDS_awards)+length(uniquePIIDS_idv),"\n\n")
#   
#   cat("Merged datasets:\n
#  Successfully Matched (AWARDS): ",nrow(ia),"
#  Successfully Matched (IDV): ",nrow(ii),"
#  Total Matched: ",nrow(ia)+nrow(ii),"
#  UniquePIIDS Awards: ",length(unique(ia$PIID)),"
#  UniquePIIDS IDV: ",length(unique(ii$PIID)),"
#  Not Matched (AWARDS): ",length(ia_piids_notmerged),"
#  Not Matched (IDV): ",length(ii_piids_notmerged),"
#  Total Not Matched: ",length(ia_piids_notmerged)+length(ii_piids_notmerged),"
#  Total Rows: ",nrow(ia)+nrow(ii)+length(ia_piids_notmerged)+length(ii_piids_notmerged),"\n\n")
#   
#   cat("Duplicate Rows (AWARDS):\n")
#   print(dupsa)
#   cat("\nDuplicate Rows (IDV):\n")
#   print(dupsi)
#   
...
#   
#   
#   if(returnUnmatched==TRUE){
#     um <- list(AWARDS=unmatched_AWARDS,
#                IDV=unmatched_IDV)
#     return(um)
#   }
#   stop()
#   
#  # cat("Duplicated Entries:
#  # Duplicated Awards Entries: ",nrow(dupsa),"
#  # Duplicated IDV Entries: ",nrow(dupsi),"
#  # Added Awards Rows: ",sum(dupsa$n),"
#  # Added IDV Rows: ",sum(dupsi$n),"
#  # If Clear Awards Duplicates Rows: ",nrow(ia)-sum(dupsa$n)+nrow(dupsa),"
#  # If Clear IDV Duplicates Rows: ",nrow(ii)-sum(dupsi$n)+nrow(dupsi),"\n\n")
#  
#  mergeEffect <- function(dups,data){
#    dups <- dups[,c("PIID","modNumber"),]
#    d <- inner_join(dups,data,by=c("PIID","modNumber"))
#    dcount <- d %>% count(PIID,modNumber,vendor_clean) %>% filter(n>1)
#    return(dcount[order(desc(dcount$n)),])
#  }
#  
#  ## I think I'm on to something here in terms of the vendorName + lastModifiedDate
#  ## I need to determine whether DOGE more closely links up with the vendorName or
#  ## the lastModifiedDate. And to see whether the lastModifiedDate can possibly be 
#  ## 'fuzzy aligned' with DOGE data. 
# 
#  ma <- mergeEffect(dupsa,ia)
#  cat("Merge Effect Trials (+vendorClean):
#  New duplicates: ",nrow(ma),"
#  New Added Rows: ",sum(ma$n),"\n\n")
#  
#  stop()
#  print("PIIDS for Awards:")
#  print(dupsa[order(desc(dupsa$n)),])
#  
#  
#   
#   stop()
#   if(verbose==T)message("Finding oddballs.")
#   ianotin <- uniquePIIDS_awards[!(uniquePIIDS_awards %in% ia$PIID)]
#   iinotin <- uniquePIIDS_idv[!(uniquePIIDS_idv %in% ii$PIID)]
#   
#   # if(length(ianotin)>0)
#   #   if(verbose==T)print(paste("Found",length(ianotin),"unmatched AWARDS PIIDs."))
#   # if(length(iinotin)>0)
#   #   if(verbose==T)print(paste("Found",length(iinotin),"unmatched IDV PIIDs."))
#   
#  cat("Summary:\n
#  DOGE Starting Unique PIIDs: ",length(unique(dc$PIID)),"
#  FPDS:AWARDS Starting Unique PIIDs: ",length(uniquePIIDS_awards),"
#  FPDS:IDV Starting Unique PIIDs: ",length(uniquePIIDS_idv),"
#  ----------------------------------------------------
#  Merged Awards Unique PIIDs: ",nrow(ia),"
#  Merged IDV Unique PIIDs: ",nrow(ii),"
#  Unmatched Award Unique PIIDs: ",length(ianotin),"
#  Unmatched IDV Unique PIIDs: ",length(iinotin),"
#  ----------------------------------------------------
#  Starting/Merged Award Diff: ",(length(uniquePIIDS_awards))-length(unique(ia$PIID)),"
#  Starting/Merged Award Diff: ",(length(uniquePIIDS_idv))-length(unique(ii$PIID)),"
#  Total Merged / Starting DOGE Diff: ",(((length(uniquePIIDS_awards))+(length(uniquePIIDS_idv)))-length(unique(dc$PIID))))
#   
#   x <- list(AWARDS=ia,
#             IDV=ii)
#   return(x)
